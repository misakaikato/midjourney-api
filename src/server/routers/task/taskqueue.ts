...
export class TaskQueue<DataType extends Record<string, any>> {
  ...
  executeTask(taskId?: string) {
    ...
    // 可执行的任务列表 (为了防止 429 code, 每次只执行一个任务)
    const canRunnableTasks = tasks.slice(
      0,
      Math.min(
        this.concurrentTaskCountLimit - this.concurrentTaskCounter,
        tasks.length,
        1
      )
    );
