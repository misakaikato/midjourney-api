...
export class MidjourneyApi extends Command {
	...
	async CustomApi({
		...
		logger.info(`[CustomApi] msgId: ${msgId}, customId: ${customId}, flags: ${flags}, payload: ${JSON.stringify(payload)}`);
		return this.safeIteractions(payload);
	}
	...
	async CustomPanImagineApi({
		...
		// const direction = customId.split("::")?.[2]?.replace("pan_", "");
		customId = customId.replace(
			`MJ::JOB::pan_right::1`,
			`MJ::PanModal::right`
		);
		customId = customId.replace("::SOLO", "");
